...
class TabMenu extends Component<MenuProps> {
    handleClickOutside = () => {
    ...
    getNavLink = ({ route, exact, text, order, className, icon }: any) => {
        return (
            <NavLink
                ...
                {/* <div className=''><Trans>{text}</Trans></div> */}
                <Trans>{text}</Trans>
            </NavLink>
        );
    };
    ...
    render() {
        const menuClass = classnames({
            'header__column mobile-menu layout_sidebar--border': true,
        ...
        return (
            <>
                <div className={menuClass}>
                    <ul className="nav nav-tabs layout_sidebar border-0 ">
                        <>
                            {MENU_ITEMS.map((item) => (
                        ...
                        <li className={`nav-item order-5`}>
                            ...
                            <CustomDropdown
                                label={this.props.t(LANGUAGES[i18n.language])}
                                baseClassName="dropdown"
                                icon="language-icon"
                                controlClassName="nav-link "
                                >
                                {Object.keys(LANGUAGES).map((lang) => (
                                    <div
                                        key={`disable_timings_${lang}`}
                                        className="dropdown-item"
                                        onClick={() => this.changeLanguageTab(lang)}>
                                        {LANGUAGES[lang]}
                                    </div>
                                ))}
                            </CustomDropdown>
...
export default withTranslation()(enhanceWithClickOutside(TabMenu));
